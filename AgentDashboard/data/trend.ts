...
export interface TrendPoint { label: string; incoming: number; engaged: number; }
...
export async function fetchTrend(webAPI: ComponentFramework.WebApi, f: ActiveFilters, bucketBy: "day" | "hour"): Promise<TrendPoint[]> {
    ...
    const query = `?$top=2000&$select=createdon,msdyn_isoutbound,_msdyn_activeagentid_value,msdyn_channel&$filter=${base}&$orderby=createdon asc`;
    const result = await webAPI.retrieveMultipleRecords("msdyn_ocliveworkitem", query);
        const buckets = new Map<string, { incoming: number; engaged: number; sortKey: number }>();
    for (const e of result.entities) {
        ...
        if (!buckets.has(key)) buckets.set(key, { incoming: 0, engaged: 0, sortKey });
        const b = buckets.get(key)!;
        if (!e["msdyn_isoutbound"]) b.incoming++;
        if (e["_msdyn_activeagentid_value"]) b.engaged++;
    }
    return Array.from(buckets.entries())
        .map(([label, v]) => ({ label, incoming: v.incoming, engaged: v.engaged, sortKey: v.sortKey }))
        .sort((a, b) => a.sortKey - b.sortKey)
        .map(({ label, incoming, engaged }) => ({ label, incoming, engaged }));
}
